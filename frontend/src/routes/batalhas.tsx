...
function BatalhasRouter() {
  const matchRoute = useMatchRoute();
  const isChild = matchRoute({ to: "/batalhas/$id" });
...
function BatalhasPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-16">
      ...
      <p className="mt-3 max-w-xl text-muted-foreground">Crie um torneio, adicione MCs e gerencie o chaveamento.</p>
      ...
      <div className="mt-12 grid gap-12 md:grid-cols-[1fr_1.2fr]">
        ...
        <form onSubmit={submit} className="space-y-4 rounded-sm border border-border bg-card p-6">
          <h2 className="font-display text-2xl tracking-widest">NOVO TORNEIO</h2>
          <Input label="Nome do torneio *" value={form.nome} onChange={(v) => setForm({ ...form, nome: v })} placeholder="Final da temporada" />
          ...
          >
            {criar.isPending ? "CRIANDO..." : "CRIAR TORNEIO"}
          </button>
        </form>
        ...
        <div>
          <div className="mb-4 flex items-baseline justify-between">
            <h2 className="font-display text-2xl tracking-widest">TORNEIOS</h2>
